...
# Forward Kinematics
    ...
    ALPHA214 = ALPHA212+BETA413*s.dpt[1,13]
    ...
    ALPHA117 = C17*(ALPHA116+BS116*s.dpt[1,16])-S17*(ALPHA316+BETA716*s.dpt[1,16])
    ALPHA217 = ALPHA216+BETA416*s.dpt[1,16]
    ALPHA317 = C17*(ALPHA316+BETA716*s.dpt[1,16])+S17*(ALPHA116+BS116*s.dpt[1,16])
    ...
    ALPHA118 = C18*(ALPHA117+BETA317*s.dpt[3,17])+S18*(ALPHA217+BETA617*s.dpt[3,17])
    ALPHA218 = C18*(ALPHA217+BETA617*s.dpt[3,17])-S18*(ALPHA117+BETA317*s.dpt[3,17])
    ALPHA318 = ALPHA317+BS917*s.dpt[3,17]
    ...
    ALPHA120 = C20*(ALPHA119+BETA319*s.dpt[3,19])-S20*(ALPHA319+BS919*s.dpt[3,19])
    ALPHA220 = ALPHA218+BETA619*s.dpt[3,19]
    ALPHA320 = C20*(ALPHA319+BS919*s.dpt[3,19])+S20*(ALPHA119+BETA319*s.dpt[3,19])
    ...
    ALPHA121 = C21*(ALPHA120+BETA220*s.dpt[2,20]+BETA320*s.dpt[3,20]+BS120*s.dpt[1,20])-S21*(ALPHA320+BETA720*s.dpt[1,20]+BETA820*s.dpt[2,20]+BS920*s.dpt[3,20])
    ALPHA221 = ALPHA220+BETA420*s.dpt[1,20]+BETA620*s.dpt[3,20]+BS520*s.dpt[2,20]
    ALPHA321 = C21*(ALPHA320+BETA720*s.dpt[1,20]+BETA820*s.dpt[2,20]+BS920*s.dpt[3,20])+S21*(ALPHA120+BETA220*s.dpt[2,20]+BETA320*s.dpt[3,20]+BS120*s.dpt[1,20])
    ...
    ALPHA122 = C22*(ALPHA121+BS121*s.dpt[1,25])-S22*(ALPHA321+BETA721*s.dpt[1,25])
    ALPHA222 = ALPHA221+BETA421*s.dpt[1,25]
    ALPHA322 = C22*(ALPHA321+BETA721*s.dpt[1,25])+S22*(ALPHA121+BS121*s.dpt[1,25])
    ...
    ALPHA123 = C23*(ALPHA120+BETA220*s.dpt[2,21]+BETA320*s.dpt[3,21]+BS120*s.dpt[1,21])-S23*(ALPHA320+BETA720*s.dpt[1,21]+BETA820*s.dpt[2,21]+BS920*s.dpt[3,21])
    ALPHA223 = ALPHA220+BETA420*s.dpt[1,21]+BETA620*s.dpt[3,21]+BS520*s.dpt[2,21]
    ALPHA323 = C23*(ALPHA320+BETA720*s.dpt[1,21]+BETA820*s.dpt[2,21]+BS920*s.dpt[3,21])+S23*(ALPHA120+BETA220*s.dpt[2,21]+BETA320*s.dpt[3,21]+BS120*s.dpt[1,21])
    ...
    ALPHA124 = C24*(ALPHA123+BS123*s.dpt[1,28])-S24*(ALPHA323+BETA723*s.dpt[1,28])
    ALPHA224 = ALPHA223+BETA423*s.dpt[1,28]
    ALPHA324 = C24*(ALPHA323+BETA723*s.dpt[1,28])+S24*(ALPHA123+BS123*s.dpt[1,28])
    BS325 = OM120*OM320
    BS625 = OM220*OM320
    BS925 = -OM120*OM120-OM220*OM220
    BETA325 = BS325+OMp220
    BETA625 = BS625-OMp120
    ALPHA125 = ALPHA120+q[25]*BETA320+(2.0)*qd[25]*OM220+BS120*s.dpt[1,24]
    ALPHA225 = ALPHA220+q[25]*BETA620-(2.0)*qd[25]*OM120+BETA420*s.dpt[1,24]
    ALPHA325 = qdd[25]+ALPHA320+q[25]*BS920+BETA720*s.dpt[1,24]
    BS326 = OM117*OM317
    BS626 = OM217*OM317
    BS926 = -OM117*OM117-OM217*OM217
    BETA326 = BS326+OMp217
    BETA626 = BS626-OMp117
    ALPHA126 = ALPHA117+(2.0)*qd[26]*OM217+BETA317*Dz263
    ALPHA226 = ALPHA217-(2.0)*qd[26]*OM117+BETA617*Dz263
    ALPHA326 = qdd[26]+ALPHA317+BS917*Dz263
    OM127 = OM117*C27-OM317*S27
    OM227 = qd[27]+OM217
    OM327 = OM117*S27+OM317*C27
    OMp127 = C27*(OMp117-qd[27]*OM317)-S27*(OMp317+qd[27]*OM117)
    OMp227 = qdd[27]+OMp217
    OMp327 = C27*(OMp317+qd[27]*OM117)+S27*(OMp117-qd[27]*OM317)
    ALPHA127 = C27*(ALPHA126+BETA326*s.dpt[3,31])-S27*(ALPHA326+BS926*s.dpt[3,31])
    ALPHA227 = ALPHA226+BETA626*s.dpt[3,31]
    ALPHA327 = C27*(ALPHA326+BS926*s.dpt[3,31])+S27*(ALPHA126+BETA326*s.dpt[3,31])
...
# Backward Dynamics
    ...
    Fq226 = Fs226+Fs227
    Fq326 = Fs326-Fs127*S27+Fs327*C27
    Cq126 = -s.trq[1,26]+s.In[1,26]*OMp117-s.In[5,26]*OM217*OM317+s.In[9,26]*OM217*OM317+Cq127*C27+Cq327*S27-Fs226*s.l[3,26]-Fs227*s.dpt[3,31]
    Cq226 = -s.trq[2,26]+Cq227+s.In[1,26]*OM117*OM317+s.In[5,26]*OMp217-s.In[9,26]*OM117*OM317+Fs126*s.l[3,26]+s.dpt[3,31]*(Fs127*C27+Fs327*S27)
    Cq326 = -s.trq[3,26]-s.In[1,26]*OM117*OM217+s.In[5,26]*OM117*OM217+s.In[9,26]*OMp317-Cq127*S27+Cq327*C27
    ...
    Cq125 = -s.trq[1,25]+s.In[1,25]*OMp120-s.In[5,25]*OM220*OM320+s.In[9,25]*OM220*OM320-Fs225*s.l[3,25]
    Cq225 = -s.trq[2,25]+s.In[1,25]*OM120*OM320+s.In[5,25]*OMp220-s.In[9,25]*OM120*OM320+Fs125*s.l[3,25]
    Cq325 = -s.trq[3,25]-s.In[1,25]*OM120*OM220+s.In[5,25]*OM120*OM220+s.In[9,25]*OMp320
    ...
    Cq223 = -s.trq[2,23]+Cq224+s.In[1,23]*OM123*OM323+s.In[5,23]*OMp223-s.In[9,23]*OM123*OM323-Fs323*s.l[1,23]-s.dpt[1,28]*(-Fs124*S24+Fs324*C24)
    Cq323 = -s.trq[3,23]-s.In[1,23]*OM123*OM223+s.In[5,23]*OM123*OM223+s.In[9,23]*OMp323-Cq124*S24+Cq324*C24+Fs223*s.l[1,23]+Fs224*s.dpt[1,28]
    ...
    Cq221 = -s.trq[2,21]+Cq222+s.In[1,21]*OM121*OM321+s.In[5,21]*OMp221-s.In[9,21]*OM121*OM321-Fs321*s.l[1,21]-s.dpt[1,25]*(-Fs122*S22+Fs322*C22)
    Cq321 = -s.trq[3,21]-s.In[1,21]*OM121*OM221+s.In[5,21]*OM121*OM221+s.In[9,21]*OMp321-Cq122*S22+Cq322*C22+Fs221*s.l[1,21]+Fs222*s.dpt[1,25]
    ...
    Fq120 = Fs120+Fs125+Fq121*C21+Fq123*C23+Fq321*S21+Fq323*S23
    Fq220 = Fq221+Fq223+Fs220+Fs225
    Fq320 = Fs320+Fs325-Fq121*S21-Fq123*S23+Fq321*C21+Fq323*C23
    Cq120 = -s.trq[1,20]+Cq125-q[25]*Fs225+s.In[1,20]*OMp120-s.In[5,20]*OM220*OM320+s.In[9,20]*OM220*OM320+Cq121*C21+Cq123*C23+Cq321*S21+Cq323*S23-Fq221*s.dpt[3,20]-Fq223*s.dpt[3,21]-Fs220*s.l[3,20]+s.dpt[2,20]*(-Fq121*S21+Fq321*C21)+s.dpt[2,21]*(-Fq123*S23+Fq323*C23)
    Cq220 = -s.trq[2,20]+Cq221+Cq223+Cq225+q[25]*Fs125+s.In[1,20]*OM120*OM320+s.In[5,20]*OMp220-s.In[9,20]*OM120*OM320+Fs120*s.l[3,20]-Fs320*s.l[1,20]-Fs325*s.dpt[1,24]-s.dpt[1,20]*(-Fq121*S21+Fq321*C21)-s.dpt[1,21]*(-Fq123*S23+Fq323*C23)+s.dpt[3,20]*(Fq121*C21+Fq321*S21)+s.dpt[3,21]*(Fq123*C23+Fq323*S23)
    Cq320 = -s.trq[3,20]+Cq325-s.In[1,20]*OM120*OM220+s.In[5,20]*OM120*OM220+s.In[9,20]*OMp320-Cq121*S21-Cq123*S23+Cq321*C21+Cq323*C23+Fq221*s.dpt[1,20]+Fq223*s.dpt[1,21]+Fs220*s.l[1,20]+Fs225*s.dpt[1,24]-s.dpt[2,20]*(Fq121*C21+Fq321*S21)-s.dpt[2,21]*(Fq123*C23+Fq323*S23)
    ...
    Cq119 = -s.trq[1,19]+s.In[1,19]*OMp119-s.In[5,19]*OM219*OM319+s.In[9,19]*OM219*OM319+Cq120*C20+Cq320*S20-Fq220*s.dpt[3,19]-Fs219*s.l[3,19]
    Cq219 = -s.trq[2,19]+Cq220+s.In[1,19]*OM119*OM319+s.In[5,19]*OMp219-s.In[9,19]*OM119*OM319+Fs119*s.l[3,19]+s.dpt[3,19]*(Fq120*C20+Fq320*S20)
    ...
    Fq117 = Fq126+Fs117+Fq118*C18-Fq219*S18
    Fq217 = Fq226+Fs217+Fq118*S18+Fq219*C18
    Fq317 = Fq318+Fq326+Fs317
    Cq117 = -s.trq[1,17]+Cq126+s.In[1,17]*OMp117-s.In[5,17]*OM217*OM317+s.In[9,17]*OM217*OM317+Cq118*C18-Cq219*S18-Fq226*Dz263-Fs217*s.l[3,17]-s.dpt[3,17]*(Fq118*S18+Fq219*C18)
    Cq217 = -s.trq[2,17]+Cq226+s.In[1,17]*OM117*OM317+s.In[5,17]*OMp217-s.In[9,17]*OM117*OM317+Cq118*S18+Cq219*C18+Fq126*Dz263+Fs117*s.l[3,17]+s.dpt[3,17]*(Fq118*C18-Fq219*S18)
    Cq317 = -s.trq[3,17]+Cq318+Cq326-s.In[1,17]*OM117*OM217+s.In[5,17]*OM117*OM217+s.In[9,17]*OMp317
    ...
    Cq216 = -s.trq[2,16]+Cq217+s.In[1,16]*OM116*OM316+s.In[5,16]*OMp216-s.In[9,16]*OM116*OM316-Fs316*s.l[1,16]-s.dpt[1,16]*(-Fq117*S17+Fq317*C17)
    Cq316 = -s.trq[3,16]-s.In[1,16]*OM116*OM216+s.In[5,16]*OM116*OM216+s.In[9,16]*OMp316-Cq117*S17+Cq317*C17+Fq217*s.dpt[1,16]+Fs216*s.l[1,16]
    ...
    Cq215 = -s.trq[2,15]+Cq216+s.In[1,15]*OM115*OM315+s.In[5,15]*OMp215-s.In[9,15]*OM115*OM315-Fs315*s.l[1,15]-s.dpt[1,15]*(-Fq116*S16+Fq316*C16)
    Cq315 = -s.trq[3,15]-s.In[1,15]*OM115*OM215+s.In[5,15]*OM115*OM215+s.In[9,15]*OMp315-Cq116*S16+Cq316*C16+Fq216*s.dpt[1,15]+Fs215*s.l[1,15]
